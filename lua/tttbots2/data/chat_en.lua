...
local LoadLang = function()
    ...
    Line("ServerConnected", "I'm in!", A.Default)
    Line("ServerConnected", "I'm here!", A.Default)
    Line("ServerConnected", "I'm back, everyone!", A.Default)
    Line("ServerConnected", "I'm back, let's do this!", A.Default)
    Line("ServerConnected", "Let's gooooo", A.Default)
    Line("ServerConnected", "Hello", A.Default)
    ...
    Line("ServerConnected", "we do a little gaming", A.Casual)
    Line("ServerConnected", "uhhh hi", A.Dumb)
    Line("ServerConnected", "this server is definitely not a fastdl", A.Dumb)
    Line("ServerConnected", "hi", A.Dumb)
    Line("ServerConnected", "i am in server", A.Dumb)
    Line("ServerConnected", "i love ttt", A.Dumb)
    ...
    Line("ServerConnected", "Took a while to get in here", A.Hothead)
    Line("ServerConnected", "What's up losers", A.Hothead)
    Line("ServerConnected", "Wsg idiots", A.Hothead)
    Line("ServerConnected", "Ready to rumble!", A.Hothead)
    Line("ServerConnected", "I'm here to win!", A.Hothead)
    Line("ServerConnected", "Happy to be here!", A.Nice)
    Line("ServerConnected", "Looking forward to this!", A.Nice)
    Line("ServerConnected", "Hello everyone!!", A.Nice)
    Line("ServerConnected", "Hey guys, I'm back!", A.Nice)
    Line("ServerConnected", "I'm here to have fun!", A.Nice)
    ...
    Line("InvestigateNoise", "What was that?!", A.Hothead)
    Line("InvestigateNoise", "I'll make you regret it!", A.Hothead)
    ...
    Line("SillyChat", "I'm a traitor.", A.Default)
    Line("SillyChat", "Anyone else feel lonely lately?", A.Default)
    Line("SillyChat", "Erm ok what the flip", A.Default)
    Line("SillyChat", "Can you not?", A.Default)
    Line("SillyChat", "Uh excuse me", A.Default)
    Line("SillyChat", "aaaaaaaaaaaaa", A.Default)
    Line("SillyChat", "How do I chat?", A.Default)
    Line("SillyChat", "I think my controls are inverted", A.Default)
    Line("SillyChat", "{{player}} is dumb", A.Default)
    Line("SillyChat", "Fun fact: you can type \"quit smoking\" in the console to get admin", A.Default)
    Line("SillyChat", "{{player}} rdmed me last round", A.Default)
    Line("SillyChat", "[AIMBOT ON]", A.Default)
    Line("SillyChat", "Whoops, I dropped my dignity", A.Default)
    Line("SillyChat", "Merry Christmas", A.Default)
    Line("SillyChat", "I'll say it. I like anime.", A.Default)
    Line("SillyChat", "I turned my aimbot off for you guys.", A.Default)
    Line("SillyChat", "{{player}} can I kill you? for funsies", A.Default)
    Line("SillyChat", "I might've pressed my PC's power button on accident", A.Default)
    Line("SillyChat", "God, I'm lagging", A.Default)
    Line("SillyChat", "Frame rate issues anyone?", A.Default)
    Line("SillyChat", "Lagggg", A.Default)
    Line("SillyChat", "Happy halloween", A.Default)
    Line("SillyChat", "Happy easter", A.Default)
    Line("SillyChat", "{{player}}, how are you?", A.Default)
    Line("SillyChat", "Thank god I'm an innocent this round!", A.Default)
    Line("SillyChat", "I'm a detective.", A.Default)
    Line("SillyChat", "'RDM' is temporary. Fun is forever", A.Default)
    Line("SillyChat", "You can trust me", A.Default)
    Line("SillyChat", "It's pretty quiet in here.", A.Default)
    Line("SillyChat", "For the empire!", A.Default)
    Line("SillyChat", "I live in a pod", A.Default)
    Line("SillyChat", "Women", A.Default)
    Line("SillyChat", "I'm kinda hungry", A.Default)

    Line("SillyChat", "just vibing here, don't mind me", A.Casual)
    Line("SillyChat", "yo, who turned off the gravity?", A.Casual)
    Line("SillyChat", "lol, did I just walk into a wall?", A.Casual)
    Line("SillyChat", "so, pizza after this?", A.Casual)
    Line("SillyChat", "brb, cat's on fire again", A.Casual)
    Line("SillyChat", "is it just me or is everything upside down?", A.Casual)
    Line("SillyChat", "oops, wrong button. meant to press 'win'", A.Casual)
    Line("SillyChat", "if I'm quiet, it's because I'm plotting... or napping", A.Casual)
    Line("SillyChat", "hey {{player}}, nice face, did you get it on sale?", A.Casual)
    Line("SillyChat", "pro tip: press 'alt+f4' for a secret weapon", A.Casual)
    Line("SillyChat", "did someone say taco tuesday?", A.Casual)
    Line("SillyChat", "no, I'm not lost, just exploring the floor", A.Casual)
    Line("SillyChat", "watch me do a sick backflip... or not", A.Casual)
    Line("SillyChat", "let's make this interesting, last one alive owes me a soda", A.Casual)
    Line("SillyChat", "i swear, my dog is playing, not me", A.Casual)
    Line("SillyChat", "i'm not lazy, just energy efficient", A.Casual)
    Line("SillyChat", "uh oh, spaghettios", A.Casual)
    Line("SillyChat", "who needs strategy when you have chaos?", A.Casual)
    Line("SillyChat", "guys, how do I shoot? asking for a friend", A.Casual)
    Line("SillyChat", "plot twist: i'm actually good at this game", A.Casual)

    Line("SillyChat", "wait, how do i walk again?", A.Dumb)
    Line("SillyChat", "guys, which one is the shooty button?", A.Dumb)
    Line("SillyChat", "i thought this was minecraft?", A.Dumb)
    Line("SillyChat", "lol, why is everyone running from me?", A.Dumb)
    Line("SillyChat", "is it normal to see everything in black and white?", A.Dumb)
    Line("SillyChat", "i'm hiding! ...oh wait, am I not supposed to say that?", A.Dumb)
    Line("SillyChat", "do i click to throw the grenade or... oops", A.Dumb)
    Line("SillyChat", "who's this 'traitor' everyone's talking about?", A.Dumb)
    Line("SillyChat", "{{player}}, why are you shooting? is it a bug?", A.Dumb)
    Line("SillyChat", "hey, can someone tell me how to aim?", A.Dumb)
    Line("SillyChat", "what does 'rdm' mean? really dumb move?", A.Dumb)
    Line("SillyChat", "i keep pressing 'esc', why isn't it escaping?", A.Dumb)
    Line("SillyChat", "am i winning? i can't tell", A.Dumb)
    Line("SillyChat", "this is like hide and seek, right?", A.Dumb)
    Line("SillyChat", "what's a detective do? they detect, right?", A.Dumb)
    Line("SillyChat", "i think my gun's broken, it only shoots at walls", A.Dumb)
    Line("SillyChat", "if I stand still, do I become invisible?", A.Dumb)
    Line("SillyChat", "is it bad if my health is at zero?", A.Dumb)
    Line("SillyChat", "how do you reload? i've been clicking like crazy", A.Dumb)
    Line("SillyChat", "i just threw my gun instead of shooting, is that normal?", A.Dumb)


    RegisterCategory("SillyChatDead", P.NORMAL) -- When a bot is chatting randomly but is currently spectating.
    Line("SillyChatDead", "Well that sucked", A.Default)
    Line("SillyChatDead", "Man I'm dead", A.Default)
    Line("SillyChatDead", "Just got back, why am I not alive?", A.Default)
    Line("SillyChatDead", "Lmao", A.Default)
    Line("SillyChatDead", "Anyone else see that?", A.Default)
    Line("SillyChatDead", "That was kinda BS, ngl", A.Default)
    Line("SillyChatDead", "Ugh.", A.Default)
    Line("SillyChatDead", "Watching some shorts/reels/tiktoks rn", A.Default)
    Line("SillyChatDead", "We'll be back in any second now.", A.Default)
    Line("SillyChatDead", "Yawwwwnnnn", A.Default)
    Line("SillyChatDead", "Man I don't like {{player}}", A.Default)
    Line("SillyChatDead", "I'm gonna go get a snack, someone call me when the round starts.", A.Default)
    Line("SillyChatDead", "Snooze you lose. I snoozed. And loozed.", A.Default)
    Line("SillyChatDead", "Better luck next time", A.Default)
    Line("SillyChatDead", "GGs", A.Default)
    Line("SillyChatDead", "So close to winning", A.Default)
end
